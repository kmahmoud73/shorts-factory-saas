...
**Last Updated**: March 19, 2026 (v36 -- **Goha Channel Card + 4 Channels**: Added Tales of Goha card (amber/gold theme, 52 stories, @TalesOfGoha, "Day 1" milestone) to results section. Grid 3→4 columns. All references updated to 4 channels: hero badge, stats number, results subtitle, FAQ channel descriptions (added Goha entry). Pushed to GitHub Pages. Previous: v35 -- **Payment Pause + Inquiry Mode + Waitlist**: Stripe checkout links removed from all 3 tier CTAs. Replaced with `openForm()` inquiry links (same contact form, just no payment). Amber "not taking new clients" notice added at top of pricing section. Inline email waitlist strip added (Formspree `mbdanpdr`, `plan: Waitlist`, `niche: waitlist-signup`). Contact modal copy updated to inquiry tone. Tiers remain fully visible. CTA opacity 0.75 (paused feel). Previous: v34 -- **Milestone Progress Bars + 3-Day Update Cadence**: Added milestone progress bars to all 3 channel cards — JV: toward 10K subs, CiT + WIL: toward 1K subs. Auto-patched by `update_site_stats.py` on each run. Added "Stats updated: [date] — refreshes every 3 days" timestamp label under results subtitle. Launchd schedule upgraded from weekly (Sunday only) to Mon + Thu + Sun at 11:35 PM. Sandy's reasoning: WIL is in growth phase, numbers move fast, potential clients deserve numbers within 72 hours. Previous: v33 -- **WIL Channel Live + Stats Refresh + Channel Log**: Website updated with real stats — JV 1350/13026/50, CiT 87/9120/53, total 22,500+. WIL card overhauled: "NEW"→"LIVE" (pulsing badge), real stats (1 video, 150+ views, @LabOfWhatIf). New expandable "Channel Log" section — terminal-style monospace feed showing 5 events newest-first: [Mar 18] avatar upgrade with before/after comparison images (lightning vs V5 clean), [Mar 17] first video, [Mar 17] pipeline activated, [Mar 13] branding, [Mar 13] channel created. Added `img/` directory with `wil_avatar_before.png` and `wil_avatar_after.png`. HeyGen V5 portrait face detection fails (cartoon style not photorealistic) — old talking photo ID kept for HeyGen, V5 clean wired into Hedra. Pushed to GitHub Pages. Previous: v32 -- Lead Responder Root Cause Fix: `_call_llm_for_reply()` rewrote to call Anthropic API directly via `requests` — no cross-directory import of `llm_client.py`, no `.pyc` chain. Root cause: launchd used venv Python, dashboard used system Python = two separate pyc caches. Clearing one didn't fix the other. New approach: ZERO external imports. Groq as fallback (also direct API via requests). `ANTHROPIC_API_KEY` + `GROQ_API_KEY` now hardcoded in plist (like SF_EMAIL_PASS). Formspree timestamp messages ("Submitted HH:MM...") now stripped before LLM/template — fixes embarrassing "You mentioned: Submitted 12:23 PM" template. Sent contextual follow-up to Meryem Gokmen (News niche). 16 leads total. Previous: v31 -- Lead Responder Hardened)
